        ...
        <form @submit.prevent="form.post(route('warehouse-receipts.store'))" class="space-y-4">
            ...
            <div class="rounded-lg bg-white p-4 shadow">
                ...
                <table class="mt-2 min-w-full">
                    <thead><tr><th class="text-left text-xs text-gray-500">{{ $t('common.product') }}</th><th class="text-left text-xs text-gray-500">{{ $t('common.qty') }}</th><th class="text-left text-xs text-gray-500">{{ $t('common.buy_price') }}</th><th class="text-left text-xs text-gray-500">{{ $t('common.sale_price') }}</th><th></th></tr></thead>
                    <tbody>
                        <tr v-for="(item, i) in form.items" :key="i" class="border-t">
                            ...
                            <td class="py-1"><input v-model.number="item.qty_received" type="number" step="0.01" min="0.01" class="w-20 rounded border-gray-300 text-sm" required /></td>
                            <td class="py-1"><input v-model.number="item.buy_price" type="number" step="0.01" min="0" class="w-24 rounded border-gray-300 text-sm" required /></td>
                            <td class="py-1"><input v-model.number="item.sale_price" type="number" step="0.01" min="0" class="w-24 rounded border-gray-300 text-sm" required /></td>
